...
def push_right_cells_after(cells, threshold):
    return [
        [
            increment_if_greater_or_equal(x0, threshold),
            y0,
            increment_if_greater_or_equal(x1, threshold),
            y1,
        ]
        for (x0, y0, x1, y1) in cells
    ]
...
def push_down_cells_after(cells, threshold):
    return [
        [
            x0,
            increment_if_greater_or_equal(y0, threshold),
            x1,
            increment_if_greater_or_equal(y1, threshold),
        ]
        for (x0, y0, x1, y1) in cells
    ]
...
class PaneCommand(sublime_plugin.WindowCommand):
    ...
    def clone_file_to_pane(self, direction, create_new_if_necessary=False):
        ...
        for s in view.sel():
            new_sel.add(s)
        sublime.set_timeout(
            lambda: new_view.set_viewport_position(view.viewport_position(), False), 0
        )
    ...
    def reorder_panes(self, leave_files_at_position=True):
        ...
        view = self.window.show_input_panel(
            "enter new index", str(old_index + 1), on_done, None, None
        )
        view.sel().clear()
    ...
    def resize_panes(self, orientation, mode):
        ...
        relevant_indx.difference_update(
            set([0, len(data) - 1])
        )  # dont show the first and last value (it's always 0 and 1)
    ...
    def _on_resize_panes_update(self, orientation, cells, relevant_indx, orig_data, text):
        layout = self._on_resize_panes_get_layout(
            orientation, cells, relevant_indx, orig_data, text
        )
...
class SaveLayoutCommand(PaneCommand, WithSettings):
    ...
    def run(self):
        self.window.show_input_panel("Window layout nickname:", "", self.on_done, None, None)
...
class AutoCloseEmptyPanes(sublime_plugin.EventListener, WithSettings):
    ...
    def on_pre_close(self, view):
        ...
        if len(window.views_in_group(active_group)) == 1:
            sublime.set_timeout(
                lambda: window.run_command("destroy_pane", {"direction": "self"}), 0
            )
